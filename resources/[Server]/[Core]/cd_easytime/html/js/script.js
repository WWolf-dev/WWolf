...
$(document).ready(function () {
    ...
    tsunamiSound = new Audio('sound/cloche.ogg');
})
...
function playSound() {
    if (tsunamiSoundAvailable) { // Check if the sound stopped playing
        ...
        tsunamiSound.volume = 1; // Set the volume to 0.5 (or adjust to your own preference)

        tsunamiSound.play().then(() => {
            tsunamiSound.loop = true;
            sleep(180000).then(() => { // Wait 3 minutes
                tsunamiSound.loop = false;
                tsunamiSoundAvailable = true;
                tsunamiSound.pause();
            });
        });
    }
